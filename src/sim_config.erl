-module(sim_config).
-export([from_file/1, xmax/1, ymax/1, numants/1, runtime/1, cell_has_food/3, cell_weight/3]).
...
iter_cell_config(Fd, Dict) ->
    case file:read_line(Fd) of
        {ok, Line} ->
            [X,Y,V] = handle_cell_line(Line),
            iter_cell_config(Fd, dict:store({X,Y}, V, Dict));
        eof -> Dict;
        {error, Reason} -> {error, Reason}
    end.
...
cell_has_food({_,_,_,_,_,Bound}, X, Y) ->
